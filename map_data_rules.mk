...
$(DATA_ASM_BUILDDIR)/maps.o: $(DATA_ASM_SUBDIR)/maps.s $(LAYOUTS_DIR)/layouts.inc $(LAYOUTS_DIR)/layouts_table.inc  \
                             $(MAPS_DIR)/headers.inc $(MAPS_DIR)/groups.inc $(MAPS_DIR)/connections.inc $(MAP_CONNECTIONS) $(MAP_HEADERS) | $(PREPROC)
	$(PREPROC) -n -c charmap.txt $< | $(CPP) -I include - | $(AS) $(ASFLAGS) -o $@
$(DATA_ASM_BUILDDIR)/map_events.o: $(DATA_ASM_SUBDIR)/map_events.s $(MAPS_DIR)/events.inc $(MAP_EVENTS) | $(PREPROC)
	$(PREPROC) -n -c charmap.txt $< | $(CPP) -I include - | $(AS) $(ASFLAGS) -o $@

$(MAPS_DIR)/%/header.inc: $(MAPS_DIR)/%/map.json | $(MAPJSON)
...
$(MAPS_DIR)/groups.inc: $(MAPS_DIR)/map_groups.json | $(MAPJSON)
...
$(LAYOUTS_DIR)/layouts.inc: $(LAYOUTS_DIR)/layouts.json | $(MAPJSON)
